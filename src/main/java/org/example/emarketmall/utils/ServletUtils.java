...
public class ServletUtils {
    ...
    public static <T> T getObjectFromPayload(HttpServletRequest req, Class<T> clazz) {
        ...
        if (!isFormUrlencodedRequest(req)) {
            ...
            System.out.println("Is JSON content type: " + isJsonContentTypeRequest(req));
            //是否为application/json
            if (isJsonContentTypeRequest(req)) {
                if (StringUtils.isNotEmpty(payload)) {
                    System.out.println("Parsing JSON payload: " + payload);
                    t = JSON.parseObject(payload, clazz);
                    System.out.println("Parsed object: " + t);
                    return t;
                }
                return null;
            }
    ...
    private static Map<String, String> getParamMapFromRequest(HttpServletRequest req) {
        ...
        //如果是JSON请求，解析请求体
        if (isJsonContentTypeRequest(req) && params.isEmpty()) {
            try {
                String json = req.getReader().lines().collect(Collectors.joining());
                if (!StringUtils.isEmpty(json)) {
                    Map<String, Object> jsonMap = JSON.parseObject(json, Map.class);
                    jsonMap.forEach((k, v) -> params.put(k, v.toString()));
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
